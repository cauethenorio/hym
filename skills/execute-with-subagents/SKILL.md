---
name: execute-with-subagents
description: Use when executing implementation plans with independent tasks in the current session. Alternative to execute-implementation — same input (approved plan), but uses fresh subagents per task with two-stage review.
---
...
3. Read CLAUDE.md and `.claude/project-instructions/execute-with-subagents.md` if it exists
...
digraph when_to_use {
    ...
    "execute-with-subagents" [shape=box];
    "execute-implementation" [shape=box];
    "Manual execution or brainstorm first" [shape=box];

    "Have implementation plan?" -> "Tasks mostly independent?" [label="yes"];
    "Have implementation plan?" -> "Manual execution or brainstorm first" [label="no"];
    "Tasks mostly independent?" -> "execute-with-subagents" [label="yes"];
    "Tasks mostly independent?" -> "execute-implementation" [label="no - tightly coupled"];
}
```

**vs. Execute Implementation (inline):**
...
```
You: I'm using Execute with Subagents to execute this plan.
...
**vs. Execute Implementation (inline):**
- Fresh context per task (no accumulated context pollution)
...
- **hym:execute-implementation** - Use for inline step-by-step execution instead of subagent-per-task
